  ...
  (function ( undefined ) {
    ...
    testFrameworks = {
      'jasmine': {
        ...
        install: function() {
          window.TestSwarm.jasmineResults = {
            error: 0,
            fail: 0,
            total: 0
          };

          jasmine.TestSwarmReporter = function() {

            this.reportRunnerResults = function(runner) {
              submit(window.TestSwarm.jasmineResults);
            };
            ...
            this.reportSpecResults = function(spec) {
              if (spec.results().failedCount > 0) {
                window.TestSwarm.jasmineResults.fail++;
              } else if (spec.results().errorCount > 0) {
                window.TestSwarm.jasmineResults.error++;
              }
              window.TestSwarm.jasmineResults.total++;
            };
          };
          jasmine.getEnv().addReporter(new jasmine.TestSwarmReporter());
          return trimSerialize();
        }
